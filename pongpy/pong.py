...
b = pygame.draw.circle(scr, c, (x, y), 10) #Ball
...
def smart_ai():
    global x, y, xa, ya, a, cy, sp, pos
    l1 = 485 - x
    point = y - (l1 * m.tan(a * m.pi / 180))
    sp = (point - (cy + pos)) / 400
    cy += sp
    if 474 < x < 475:
        print(f'{pos=}, {cy=}, {l1=}, {a=}, cy + pos={cy + pos}, {y=}, {point=}, {sp=}')
def physics():
